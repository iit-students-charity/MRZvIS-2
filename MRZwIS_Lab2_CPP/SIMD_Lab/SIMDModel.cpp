		...
		for (int j = 0; j < q; j++)
		{
			for (int k = 0; k < m; k++)
			{
				...
				case MIN_AB:
					...
					break;
				case SUP_AB:
					buffer = subprocessorList[subprocessorIndex]->run(MIN, 1 - matrix1->getElement(i, k), 0);
					updateTime(MIN);
					resultMatrix->setElement(i, j, k,
					...
					break;
				case SUP_BA:
					buffer = subprocessorList[subprocessorIndex]->run(MIN, 1 - matrix1->getElement(k, j), 0);
					updateTime(MIN);
					resultMatrix->setElement(i, j, k,
	...
	for (int i = 0; i < p; i++)
	{
		for (int j = 0; j < q; j++)
		{
			...
			case MULTIPLICATION_F:
				for (int k = 0; k < m; k++)
				{
					resultMatrix->setElement(i, j,
						subprocessorList[subprocessorIndex]->run(MULTIPLY, resultMatrix->getElement(i, j), matrix->getElement(i, j, k)));
					switchSubprocessorIndex(&subprocessorIndex);
					updateTime(MULTIPLY);
				}
				break;
			case MULTIPLICATION_D:
				...
				{
					resultMatrix->setElement(i, j,
						subprocessorList[subprocessorIndex]->run(MULTIPLY, resultMatrix->getElement(i, j), 1 - matrix->getElement(i, j, k)));
					switchSubprocessorIndex(&subprocessorIndex);
					updateTime(MULTIPLY);
				}
...
void SIMDModel::calculateCMatrix()
{
	...
	buffer[2] = processTwoDim(MULTIPLY, buffer[2], buffer[3]);
